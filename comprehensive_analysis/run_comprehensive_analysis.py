...
def create_comprehensive_summary(feature_results, algorithm_results):
    ...
    fig.suptitle('RQ1: Comprehensive Neuromorphic Anomaly Detection Analysis\nFeature Engineering + Algorithm Approach', 
    ...
    if feature_results:
        ...
        for feature_type in feature_types:
            f1_scores = [feature_results[feature_type][clf]['f1'] for clf in feature_results[feature_type]]
            avg_f1_by_feature[feature_type] = np.mean(f1_scores)
        
        colors = ['skyblue', 'lightcoral', 'lightgreen']
        axes[0].bar(feature_types, list(avg_f1_by_feature.values()), color=colors, alpha=0.8)
        ...
        axes[0].text(0.5, 0.5, 'Part A\nFailed', ha='center', va='center', transform=axes[0].transAxes, fontsize=14)
        best_feature = "Unknown"
        best_feature_score = 0
    
    # 2. Algorithm comparison summary
    if algorithm_results:
        supervised_f1 = [algorithm_results['supervised_classification'][clf]['f1'] 
                        for clf in algorithm_results['supervised_classification']]
        unsupervised_f1 = [algorithm_results['unsupervised_anomaly_detection'][clf]['f1'] 
        ...
        all_algorithms = {}
        for clf in algorithm_results['supervised_classification']:
            all_algorithms[f'Supervised_{clf}'] = algorithm_results['supervised_classification'][clf]['f1']
        for clf in algorithm_results['unsupervised_anomaly_detection']:
            all_algorithms[f'Unsupervised_{clf}'] = algorithm_results['unsupervised_anomaly_detection'][clf]['f1']
        
        best_algorithm = max(all_algorithms, key=all_algorithms.get)
        best_algorithm_score = all_algorithms[best_algorithm]
    else:
        axes[1].text(0.5, 0.5, 'Part B\nFailed', ha='center', va='center', transform=axes[1].transAxes, fontsize=14)
...
def main():
    ...
    if feature_results or algorithm_results:
        ...
        if feature_results and algorithm_results:
            ...
            print(f"• Best Features: {summary['best_feature']} (F1={summary['best_feature_score']:.3f})")
            print(f"• Best Approach: {summary['best_approach'].replace(chr(10), ' ')} (F1={summary['best_approach_score']:.3f})")
            print(f"• Best Overall: {summary['best_algorithm'].replace('_', ' ')} (F1={summary['best_algorithm_score']:.3f})")

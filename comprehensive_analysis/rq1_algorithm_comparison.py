...
def run_algorithm_comparison_experiment():
    ...
    supervised_classifiers = {
        ...
        "Logistic Regression": LogisticRegression(
            random_state=SEED, max_iter=2000, C=10
        ),
    ...
    for name, clf in supervised_classifiers.items():
        ...
        try:
            ...
            print(
                f"  {name}: F1={metrics['f1']:.3f}, AUC={metrics['auc']:.3f}, Time={train_time:.3f}s"
            )
    ...
    for name, detector in anomaly_detectors.items():
        ...
        try:
            if name == "Local Outlier Factor":
                ...
                y_pred = (y_pred_scores < 0).astype(
                    int
                )  # LOF: negative scores are outliers
                ...
                y_pred = (y_pred_raw == -1).astype(
                    int
                )  # Convert -1 (outlier) to 1 (anomaly)
                y_pred_scores = detector.decision_function(X_all_scaled)
...
def create_algorithm_comparison_visualization(results):
    ...
    supervised_f1 = [
        results["supervised_classification"][clf]["f1"]
        for clf in results["supervised_classification"]
    ]
    supervised_auc = [
        results["supervised_classification"][clf]["auc"]
        for clf in results["supervised_classification"]
    ]
    unsupervised_f1 = [
        results["unsupervised_anomaly_detection"][clf]["f1"]
        for clf in results["unsupervised_anomaly_detection"]
    ]
    unsupervised_auc = [
        results["unsupervised_anomaly_detection"][clf]["auc"]
        for clf in results["unsupervised_anomaly_detection"]
    ]
    ...
    axes[0, 0].bar(
        x - width / 2, avg_f1, width, label="F1-Score", alpha=0.8, color="skyblue"
    )
    axes[0, 0].bar(
        x + width / 2, avg_auc, width, label="AUC", alpha=0.8, color="lightcoral"
    )
    ...
    all_algorithms = list(results["supervised_classification"].keys()) + list(
        results["unsupervised_anomaly_detection"].keys()
    )
    all_f1_scores = [
        results["supervised_classification"][clf]["f1"]
        for clf in results["supervised_classification"]
    ...
    colors = ["skyblue"] * len(results["supervised_classification"]) + [
        "lightgreen"
    ] * len(results["unsupervised_anomaly_detection"])
    ...
    legend_elements = [
        Patch(facecolor="skyblue", label="Supervised"),
    ...
    all_results = {}
    for clf in results["supervised_classification"]:
        all_results[f"Supervised_{clf}"] = results["supervised_classification"][clf][
            "f1"
        ]
    for clf in results["unsupervised_anomaly_detection"]:
        all_results[f"Unsupervised_{clf}"] = results["unsupervised_anomaly_detection"][
            clf
        ]["f1"]
    ...
    # Color by approach
    top_colors = [
        "skyblue" if "Supervised" in name else "lightgreen"
        for name, _ in top_performers
    ]
    ...
    best_supervised = max(
        results["supervised_classification"].keys(),
    ...
    best_unsupervised_f1 = results["unsupervised_anomaly_detection"][best_unsupervised][
        "f1"
    ]
    ...
    if np.mean(unsupervised_f1) > np.mean(supervised_f1):
        improvement = (
            (np.mean(unsupervised_f1) - np.mean(supervised_f1)) / np.mean(supervised_f1)
        ) * 100
        print(
            f"\n💡 INSIGHT: Unsupervised anomaly detection provides {improvement:.1f}% improvement"
        )
        print(
            "• Learning 'normal' patterns is more effective than binary classification"
        )
        ...
        improvement = (
            (np.mean(supervised_f1) - np.mean(unsupervised_f1))
            / np.mean(unsupervised_f1)
        ) * 100
        print(
...
if __name__ == "__main__":
    try:
        ...
        pd.DataFrame(algorithm_summary).to_csv(
            "algorithm_comparison_results.csv", index=False
        )
        print(
            "✅ Algorithm comparison results saved to 'algorithm_comparison_results.csv'"
        )

...
def load_mvsec_data(data_path='../data', sequence='indoor_flying', camera='left'):
    ...
    print(f"📊 Total loaded: {len(combined_events_data):,} events from {len(data_files)} file(s)")
...
class SpatiotemporalFeatureExtractor:
    ...
    def extract_features(self, frame, prev_frame=None):
        ...
        if prev_frame is not None:
            ...
            flow_coherence = 1.0 / (1.0 + np.std(flow_angle)) if np.std(flow_angle) > 1e-6 else 1.0
        else:
            flow_mag_mean = flow_mag_std = flow_angle_mean = flow_angle_std = flow_coherence = 0.0
...
class NeuromorphicFeatureExtractor:
    ...
    def compute_fractal_dimension(self, frame):
        ...
        H, W = binary.shape
        max_size = min(H, W) // 4
        sizes = [2**i for i in range(1, int(np.log2(max_size)) + 1) if 2**i <= max_size]
    ...
    def extract_features(self, frame, prev_frame=None):
        ...
        if prev_frame is not None:
            ...
            prev_combined = np.sum(prev_frame, axis=0) if prev_frame.shape[0] > 1 else prev_frame[0]
            ...
            motion_consistency = 1.0 / (1.0 + np.std(motion_angle)) if np.std(motion_angle) > 1e-6 else 1.0
        ...
        remaining_features = [lifetime_mean, lifetime_std, burst_count, silence_count,
                            lbp_mean, lbp_std, fractal_dim, 0, 0, 0, 0, 0, 0]
...
def run_feature_comparison_experiment():
    ...
    for feature_name, feature_data in feature_sets.items():
        ...
        for clf_name, clf in classifiers.items():
            ...
            y_prob = clf_copy.predict_proba(X_test_scaled)[:, 1] if hasattr(clf_copy, 'predict_proba') else y_pred
    ...
    create_feature_comparison_visualization(results, basic_time, spatio_time, neuro_time)
    
    return results


def create_feature_comparison_visualization(results, basic_time, spatio_time, neuro_time):
    """Create visualization focused on feature comparison"""
    fig, axes = plt.subplots(2, 2, figsize=(15, 10))
    fig.suptitle('RQ1-A: Feature Engineering Impact Analysis\nWhich features work best for neuromorphic anomaly detection?', 
    ...
    for feature_type in feature_types:
        ...
        for metric in metrics:
            scores = [results[feature_type][clf][metric] for clf in results[feature_type]]
    ...
    for i, feature_type in enumerate(feature_types):
        ...
        axes[0,0].bar(x + i*width, scores, width, label=feature_type, 
    ...
    for i, (ft, time_val) in enumerate(zip(feature_types, times)):
        ratio = time_val / basic_time if basic_time > 0 else 1
        axes[0,1].text(i, time_val + max(times) * 0.05, f'{ratio:.1f}x', 
    ...
    for i, feature_type in enumerate(feature_types):
        ...
        axes[1,0].bar(x + i*width, f1_scores, width, label=feature_type,
                     alpha=0.8, color=colors[i])
    ...
    improvements = {
        'Spatiotemporal': ((spatio_avg_f1 - basic_avg_f1) / basic_avg_f1 * 100) if basic_avg_f1 > 0 else 0,
        'Neuromorphic': ((neuro_avg_f1 - basic_avg_f1) / basic_avg_f1 * 100) if basic_avg_f1 > 0 else 0
    }
    
    axes[1,1].bar(list(improvements.keys()), list(improvements.values()), 
    ...
    for i, (feature, improvement) in enumerate(improvements.items()):
        axes[1,1].text(i, improvement + (max(improvements.values()) * 0.05 if improvement > 0 else -max(abs(min(improvements.values())), 1) * 0.05), 
                      f'{improvement:.1f}%', ha='center', va='bottom' if improvement > 0 else 'top', fontweight='bold')
    ...
    print(f"F1 Scores: Basic={basic_avg_f1:.3f}, Spatio={spatio_avg_f1:.3f}, Neuro={neuro_avg_f1:.3f}")
    print(f"Efficiency: Basic=1.0x, Spatio={spatio_time/basic_time:.1f}x, Neuro={neuro_time/basic_time:.1f}x")
...
if __name__ == "__main__":
    try:
        ...
        pd.DataFrame(feature_summary).to_csv('feature_comparison_results.csv', index=False)
